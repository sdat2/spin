...
def acc_fun_plotter():
    ...
    for radius_index in range(len(radii)):
        a_values.append([])
        test_particle = [Particle(np.array([0, 0, 0]), np.array([0, 0, 0]),
                                  1.0, True, 0, halo=True, radius=radii[radius_index])]
        ...
        plt.plot(x_values, a_values[radius_index], LineWidth=0.5,
