...
def plot_energy(co, sy):
    ...
    plt.plot(
        sy.short_timer, sy.Total_Energies["Gravitational"], label="GPE", LineWidth=1
    )
    plt.plot(
        sy.short_timer, sy.Total_Energies["Total"], label="Total Energy", LineWidth=1
    )
...
def plot_angular_momentum(co, sy):
    ...
    print("giving it the directory with name " + co.out)
    plt.plot(
        sy.short_timer, sy.Angular_Momentum["Total"][2], label="Total AM", LineWidth=1
    )
    x_lim = [np.min(sy.short_timer), np.max(sy.short_timer)]
    plt.xlim(x_lim)  # Don't waste length.
    plt.plot(x_lim, [0, 0], color="k", LineWidth=1)  # Put an origin line in
...
def plot_multi_AM(co, sy):
    ...
    for i in sy.Angular_Momentum:
        plt.plot(
            sy.short_timer, sy.Angular_Momentum[i][2], label="AM " + str(i), LineWidth=1
        )
    x_lim = [np.min(sy.short_timer), np.max(sy.short_timer)]
    plt.xlim(x_lim)  # Don't waste length.
    plt.plot(x_lim, [0, 0], color="k", LineWidth=1)  # Put an origin line in
