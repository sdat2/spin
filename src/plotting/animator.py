...
def _read_in(OUTPUT_DIREC, name):
    '''load pickles'''
...
class AniMP4:
    ...
    @timeit
    def __init__(self, OUTPUT_DIREC,
                 name='forgot_name_TC_Animator',
                 move_with=True, **kwargs):
        '''load the pickle files into memory,
         then graphing objects to self'''
        self.co, self.sy = _read_in(OUTPUT_DIREC, name)
        ...
        with plt.style.context(('dark_background')):
            ...
            if self.no_particles <10: # Deal with multi body animate test case
                self.line, = self.ax.plot([], [], 'wo',
                                          markersize=1, label='Test Masses')
                self.galactic_centre, = self.ax.plot([0], [0], 'wo',
                                                     markersize=1,
                                                     label='Galaxy A')
                self.impactor, = self.ax.plot([0], [0], 'wo',
                                              markersize=1,
                                              label='Galaxy B')
            else:
                self.line, = self.ax.plot([], [], 'wo', markersize=0.5,
                                          label='Test Masses')
                self.galactic_centre, = self.ax.plot([0], [0],
                                                     color='red',
                                                     marker='+',
                                                     label='Galaxy A')
                self.impactor, = self.ax.plot([0], [0],
                                              color='green',
                                              marker='+',
                                              label='Galaxy B')
            ...
            self.time_text = self.ax.text(1.01, 0.95, '',
                                          transform=self.ax.transAxes)
            self.KE_text = self.ax.text(1.01, 0.88, '',
                                        transform=self.ax.transAxes)
            self.GPE_text = self.ax.text(1.01, 0.81, '',
                                         transform=self.ax.transAxes)
            self.energy_text = self.ax.text(1.01, 0.74, '',
                                            transform=self.ax.transAxes)
            plt.tight_layout() # This supposedly makes stops the label from falling off.

        print('Timer is '+ str(len(self.sy.short_timer))+' Long')
        self.dt = self.sy.timer[1]-self.sy.timer[0] # read the time step directly from the timer file
    ...
    def add_details(self):
        ...
        self.algorithm_title = self.ax.text(1.01, 0.65,'Algorithm:',
                                            transform=self.ax.transAxes)
        self.algorithm_text = self.ax.text(1.01, 0.58, algo,
                                           transform=self.ax.transAxes)
        self.timestep_text = self.ax.text(1.01, 0.51, 'dt =' +str(self.co.tstep),
                                          transform=self.ax.transAxes)
        self.length_softening_distance = self.ax.text(1.01, 0.44, r'$\epsilon$ = '
                                                      + str(self.co.epsilon),
                                                      transform=self.ax.transAxes)
    ...
    def ani_init(self):
        ...
        return self.line, self.galactic_centre,\
               self.impactor, self.time_text,\
               self.KE_text, self.GPE_text,\
               self.energy_text
    ...
    def animate(self, i):
        ...
        self.time_text.set_text('Time = %.1f' %
                                self.sy.short_timer[i])
        self.KE_text.set_text('KE = %.6f' %
                              self.sy.Total_Energies['Kinetic'][i])
        self.GPE_text.set_text('GPE = %.6f' %
                               self.sy.Total_Energies['Gravitational'][i])
        self.energy_text.set_text('E = %.6f' %
                                  self.sy.Total_Energies['Total'][i])
        plt.tight_layout() # This supposedly makes stops the label from falling off.
        a = self.sy.coordinate_grid[0, 0, i]
        b = self.sy.coordinate_grid[1, 0, i]
        if self.move_with == False: # inertial frame
            self.line.set_data(self.sy.coordinate_grid[0, :, i],
                               self.sy.coordinate_grid[1, :, i])
            self.galactic_centre.set_data(a, b)
            self.impactor.set_data(self.sy.coordinate_grid[0, -1, i],
                                   self.sy.coordinate_grid[1, -1, i])
        if self.move_with == True: # comoving frame
            self.line.set_data(self.sy.coordinate_grid[0, :, i] - a,
                               self.sy.coordinate_grid[1, :, i] - b)
            self.galactic_centre.set_data(0.0, 0.0)
            self.impactor.set_data(self.sy.coordinate_grid[0, -1, i] - a,
